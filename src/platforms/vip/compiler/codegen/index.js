...
export function generate(
    ...
    const code = ast ? genElement(ast) : '$ctx->_c("div")'
...
function genFor (el: any): string {
  const exp = el.for
  const alias = varExport(el.alias);
  const iterator1 = el.iterator1 ? varExport(el.iterator1) : 'null'
  const iterator2 = el.iterator2 ? varExport(el.iterator2) : 'null'
...
function genDirectives (el: ASTElement): string | void {
  ...
  for (i = 0, l = dirs.length; i < l; i++) {
    ...
    if (needRuntime) {
      hasRuntime = true
      res += `array("name"=>"${dir.name}", "rawName"=>"${dir.rawName}"${
        dir.value ? `,"value"=>(${transformExpressionNoFilter(dir.value)}),"expression"=>${varExport(dir.value)}` : ''
      }${
        dir.arg ? `,"arg"=>"${dir.arg}"` : ''
      }${
        dir.modifiers ? `,"modifiers"=>${varExport(dir.modifiers)}` : ''
      }),`
    }
  }
  if (hasRuntime) {
...
function genInlineTemplate (el: ASTElement): ?string {
  ...
  if (ast.type === 1) {
    ...
    return `"inlineTemplate"=>array("render" => function($ctx){${
      inlineRenderFns.render
    }},"staticRenderFns"=>array(${
      inlineRenderFns.staticRenderFns.map(code => `function($ctx){${code}}`).join(',')
    }))`
  }
}

function genScopedSlots (slots: { [key: string]: ASTElement }): string {
  return `"scopedSlots"=>$ctx->_u(array(${
...
function genChildren (el: ASTElement, checkSkip?: boolean): string | void {
  ...
  if (children.length) {
    ...
    return `array(${children.map(genNode).join(',')})${
...
function genSlot (el: ASTElement): string {
  ...
  let res = `$ctx->_t(${slotName}${children ? `,${children}` : ''}`
  const attrs = el.attrs && `array(${el.attrs.map(a => `"${camelize(a.name)}"=>${(a.value)}`).join(',')})`
...
function genComponent (componentName: string, el: ASTElement): string {
  const children = el.inlineTemplate ? null : genChildren(el, true)
  return `$ctx->_c(${componentName},${genData(el)}${
    children ? `,${children}` : ''
  })`
}
