...
var vuescript = require("./vuescript");

function transformExpressionNoFilter(exp) {
    var ast = vuescript.parse(exp, {
        startRule: "VueExpressionNoFilter"
    });
    var result = genCode(ast);
    return result.code;
}
...
function parseText (
  ...
  while ((match = tagRE.exec(text))) {
    ...
    // tag token
    var exp = transformExpression(match[1].trim());
    tokens.push(("$ctx->_s(" + exp + ")"));
...
function generate(
    ...
    var code = ast ? genElement(ast) : '$ctx->_c("div")';
...
function genElement(el) {
    if (el.staticRoot && !el.staticProcessed) {
        ...
        if (el.component) {
            ...
            code = "$ctx->_c('" + (el.tag) + "'" + (data ? ("," + data) : ',null') + (children ? ("," + children) : ',null') + ")";
    }
    // module transforms
...
function genStatic (el) {
  ...
  return ("$ctx->_m(" + (staticRenderFns.length - 1) + (el.staticInFor ? ',true' : '') + ")")
}
...
function genOnce (el) {
  ...
  if (el.if && !el.ifProcessed) {
    ...
    return ("$ctx->_o(" + (genElement(el)) + "," + (onceCount++) + (key ? ("," + key) : "") + ")")
  } else {
    return genStatic(el)
  }
}
...
function genIfConditions (conditions) {
  ...
  if (condition.exp) {
    return ("(" + (transformExpressionNoFilter(condition.exp)) + ")?" + (genTernaryExp(condition.block)) + ":" + (genIfConditions(conditions)))
  } else {
...
function genFor (el) {
  var exp = el.for;
  var alias = varExport(el.alias);
  var iterator1 = el.iterator1 ? varExport(el.iterator1) : 'null';
  var iterator2 = el.iterator2 ? varExport(el.iterator2) : 'null';
  ...
  return "$ctx->_l(" + (transformExpressionNoFilter(exp)) + "," +
...
function genData (el) {
  ...
  // attributes
  if (el.attrs) {
    data += "\"attrs\"=>array(" + (genProps(el.attrs)) + "),";
  }
  // DOM props
  if (el.props) {
  ...
  // component v-model
  if (el.model) {
    data += "\"model\" => array(\"value\" => " + (el.model.value) + ", \"callback\" => " + (el.model.callback) + "),";
    // FIXME expressio 是否有用
    // data += `model:{value:${
    //   el.model.value
    // },callback:${
    //   el.model.callback
...
function genDirectives (el) {
  ...
  for (i = 0, l = dirs.length; i < l; i++) {
    ...
    if (needRuntime) {
      hasRuntime = true;
      res += "array(\"name\"=>\"" + (dir.name) + "\", \"rawName\"=>\"" + (dir.rawName) + "\"" + (dir.value ? (",\"value\"=>(" + (transformExpressionNoFilter(dir.value)) + "),\"expression\"=>" + (varExport(dir.value))) : '') + (dir.arg ? (",\"arg\"=>\"" + (dir.arg) + "\"") : '') + (dir.modifiers ? (",\"modifiers\"=>" + (varExport(dir.modifiers))) : '') + "),";
    }
  }
  if (hasRuntime) {
...
function genInlineTemplate (el) {
  ...
  if (ast.type === 1) {
    ...
    return ("\"inlineTemplate\"=>array(\"render\" => function($ctx){" + (inlineRenderFns.render) + "},\"staticRenderFns\"=>array(" + (inlineRenderFns.staticRenderFns.map(function (code) { return ("function($ctx){" + code + "}"); }).join(',')) + "))")
  }
}

function genScopedSlots (slots) {
  return ("\"scopedSlots\"=>$ctx->_u(array(" + (Object.keys(slots).map(function (key) { return genScopedSlot(key, slots[key]); }).join(',')) + "))")
}
...
function genChildren (el, checkSkip) {
  ...
  if (children.length) {
    ...
    return ("array(" + (children.map(genNode).join(',')) + ")" + (normalizationType ? ("," + normalizationType) : ''))
  }
}
...
function genSlot (el) {
  ...
  var res = "$ctx->_t(" + slotName + (children ? ("," + children) : '');
  var attrs = el.attrs && ("array(" + (el.attrs.map(function (a) { return ("\"" + (camelize(a.name)) + "\"=>" + ((a.value))); }).join(',')) + ")");
...
function transformNode (el, options) {
  ...
  if (process.env.NODE_ENV !== 'production' && staticClass) {
    var expression = parseText$1(staticClass, options.delimiters);
    if (expression) {
      warn(
        "class=\"" + staticClass + "\": " +
        'Interpolation inside attributes has been removed. ' +
...
function transformNode$1 (el, options) {
  ...
  if (staticStyle) {
    /* istanbul ignore if */
    if (process.env.NODE_ENV !== 'production') {
      var expression = parseText$1(staticStyle, options.delimiters);
      if (expression) {
        warn(
          "style=\"" + staticStyle + "\": " +
          'Interpolation inside attributes has been removed. ' +
...
function isType (type, fn) {
  if (!Array.isArray(fn)) {
    return getType$1(fn) === getType$1(type)
  }
  for (var i = 0, len = fn.length; i < len; i++) {
    if (getType$1(fn[i]) === getType$1(type)) {
      return true
    }
  }
  /* istanbul ignore next */
